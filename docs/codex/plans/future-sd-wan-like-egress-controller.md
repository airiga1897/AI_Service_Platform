...
egress paths for operator-approved targets. The first production-safe version is
selective and profile-based: it heals only domains/IPs listed in
`operator/egress_policy/profiles.json`, using active cascade topology as the
allowed path set.
