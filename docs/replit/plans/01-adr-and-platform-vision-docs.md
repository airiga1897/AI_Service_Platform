...
## What & Why
Зафиксировать в репозитории долгосрочное видение платформы и завести лёгкий процесс ADR (Architecture Decision Records), чтобы все принятые архитектурные решения были найдены в одном месте и могли пересматриваться по мере развития.
...
- В `README.md` добавлен раздел «Architecture Decision Records» со ссылкой на `docs/adr/README.md` и краткое перечисление 4 рантайм-инстансов.
- В `docs/ARCHITECTURE.md` добавлен раздел «Roadmap / Runtime instances», описывающий 4 текущих инстанса и план добавления ботов/новых сайтов; есть ссылка на `services.yml` как на источник истины.
...
## Steps
1. **Каркас ADR** — создать `docs/adr/`, `docs/adr/README.md` (индекс) и `docs/adr/template.md` (MADR-шаблон: Context, Decision, Status, Consequences, Date).
...
4. **Кросс-ссылки** — добавить раздел «Architecture Decision Records» в `README.md` и раздел «Roadmap / Runtime instances» в `docs/ARCHITECTURE.md`; обновить индекс ADR.
