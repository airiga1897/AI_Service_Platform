param(
    ...
    [ValidateSet("plan", "apply", "absent", "purge", "reseed", "probe", "stage-image", "stage-support-images", "publication-check", "backup-init", "backup-schedule", "backup", "restore-rehearsal", "restore-cleanup")]
    [string]$Action,
...
if (-not $BatchPlanFile -and $Service -eq "site_runtime" -and $Action -notin @("plan", "probe", "stage-image", "stage-support-images", "publication-check", "apply", "backup-init", "backup-schedule", "backup", "restore-rehearsal", "restore-cleanup")) {
...
if (-not $BatchPlanFile -and $Service -eq "site_runtime" -and $Action -eq "publication-check") {
    if (-not $Instance -or -not $Limit) {
        Fail "site_runtime publication-check requires -Instance and exactly one -Limit alias"
    }
    if (-not $Check) {
