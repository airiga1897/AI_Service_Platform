...
GEO_POLICY_TOOLS_DIR="tools/geo_policy"
...
usage() {
    cat <<'USAGE'
Usage:
  bash tools/services/service_remote.sh <service> <plan|apply|rollback|absent|purge|reseed|probe|stage-image|stage-support-images> [options]
...
Options:
  ...
  --geo-policy-active-path PATH
                        geo_policy only: explicit configured egress alias.
...
case "$ACTION" in
    plan|apply|rollback|absent|purge|reseed|probe|stage-image|stage-support-images) ;;
    *) usage; fail "Action must be one of: plan, apply, rollback, absent, purge, reseed, probe, stage-image, stage-support-images" ;;
esac
if [ "$SERVICE" = "vpn" ]; then
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --geo-policy-active-path) GEO_POLICY_ACTIVE_PATH="${2:-}"; shift 2 ;;
...
if [ "$SERVICE" = "geo_policy" ]; then
    ...
    [ "$LIMIT" = "vps3" ] || fail "geo_policy canary requires exactly --limit vps3"
    if [ "$ACTION" = "rollback" ] && [ "$CHECK" = "true" ]; then
...
[ -d "$GEO_POLICY_TOOLS_DIR" ] || fail "GeoPolicy tools directory not found: $GEO_POLICY_TOOLS_DIR"
...
remote_geo_policy_tools_temp="$remote_bundle_dir/tools/geo_policy"
...
[ "$SERVICE" = "geo_policy" ] && remote_args+=("--geo-policy-active-path" "$(quote_bash_arg "$GEO_POLICY_ACTIVE_PATH")")
...
cp -a "$GEO_POLICY_TOOLS_DIR" "$staging_dir/tools/geo_policy"
...
for operator_subdir in haproxy softether edge_banlist postgres platform_networks host_resources platform_router geo_policy site_runtime; do
...
run_stage $(quote_bash_arg "remove previous GeoPolicy tools") sudo rm -rf $(quote_bash_arg "$REMOTE_REPO_DIR/tools/geo_policy")
run_stage $(quote_bash_arg "install GeoPolicy tools") sudo cp -a $(quote_bash_arg "$remote_geo_policy_tools_temp") $(quote_bash_arg "$REMOTE_REPO_DIR/tools/geo_policy")
...
if [ -d $(quote_bash_arg "$remote_operator_temp/geo_policy") ]; then run_stage $(quote_bash_arg "sync operator geo_policy config") sudo bash -lc $(quote_bash_arg "rm -rf $(quote_bash_arg "$remote_operator_dir/geo_policy"); cp -a $(quote_bash_arg "$remote_operator_temp/geo_policy") $(quote_bash_arg "$remote_operator_dir/geo_policy"); chown -R ansible:ansible $(quote_bash_arg "$remote_operator_dir/geo_policy")"); fi
